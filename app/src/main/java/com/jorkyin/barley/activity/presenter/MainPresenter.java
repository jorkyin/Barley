...
public class MainPresenter {
    ...
    public void onStart(ArrayList<P2PParam> mData) {
        for (int i = 0; i < mData.size(); i++) {
            if (Const.PPPP_STATUS_ON_LINE != mData.get(i).getStateValue()) {
                NativeCaller.StartConnectDev(mData.get(i).getUID(), mData.get(i).getUser(), mData.get(i).getPassword(), "");
            }
        }
    }

    public void p2pConnectDev(P2PParam p2PParam) {
        if (Const.PPPP_STATUS_ON_LINE != p2PParam.getStateValue()) {
            NativeCaller.StartConnectDev(p2PParam.getUID(), p2PParam.getUser(), p2PParam.getPassword(), "");
        }
    }
